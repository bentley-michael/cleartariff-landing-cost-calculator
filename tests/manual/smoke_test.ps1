...
$artifactDir = Join-Path $project "tests\artifacts"
...
$testFile = Join-Path $artifactDir ("TariffTest_{0:yyyyMMdd_HHmmss}.pdf" -f (Get-Date))
...
if ((Test-Path $testFile) -and ((Get-Item $testFile).Length -gt 0)) {
  ...
  Write-Host "FAIL /pdf/weasy-test" -ForegroundColor Red
}
...
$orderFile = Join-Path $artifactDir ("TariffOrder_{0:yyyyMMdd_HHmmss}.pdf" -f (Get-Date))
...
if ((Test-Path $orderFile) -and ((Get-Item $orderFile).Length -gt 0)) {
  ...
  Write-Host "FAIL /generate" -ForegroundColor Red
}
